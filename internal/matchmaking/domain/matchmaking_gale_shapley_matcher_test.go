...
func TestGaleShapleyStableMatcher_FindStableMatches(t *testing.T) {
	...
	t.Run("unrestricted preferences scenario", func(t *testing.T) {
		...
		userB1 := domain.NewUser("B1", 25, gender.GenderUnspecified, domain.DefaultPreferences())
		userB2 := domain.NewUser("B2", 30, gender.GenderUnspecified, domain.DefaultPreferences())

		matcher := domain.NewGaleShapleyStableMatcher()
		userA1 := domain.NewUser("A1", 20, gender.GenderUnspecified, domain.DefaultPreferences())
	...
	t.Run("multiple A and B with partial compatibility", func(t *testing.T) {
		...
		prefsB1 := domain.DefaultPreferences()
		prefsB2 := domain.DefaultPreferences()
		...
		userA1 := domain.NewUser("A1", 20, gender.GenderUnspecified, domain.DefaultPreferences())
		userA2 := domain.NewUser("A2", 24, gender.GenderUnspecified, domain.DefaultPreferences())
	...
	t.Run("no compatibility found", func(t *testing.T) {
		// B1 and B2
		userB1 := domain.NewUser("B1", 25, gender.GenderUnspecified, domain.DefaultPreferences())
		userB2 := domain.NewUser("B2", 60, gender.GenderMale, domain.DefaultPreferences())

		matcher := domain.NewGaleShapleyStableMatcher()

		// A1 and A2 have default preferences, assuming no additional constraints
		userA1 := domain.NewUser("A1", 20, gender.GenderMale, domain.DefaultPreferences().WithMaxAge(18))
		userA2 := domain.NewUser("A2", 24, gender.GenderFemale, domain.DefaultPreferences().WithMaxAge(18))
