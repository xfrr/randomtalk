...
func TestGaleShapleyStableMatcher_FindStableMatches(t *testing.T) {
	...
	t.Run("unrestricted preferences scenario", func(t *testing.T) {
		...
		userB1 := domain.NewUser("B1", 25, gender.Unspecified, matchmaking.DefaultPreferences())
		userB2 := domain.NewUser("B2", 30, gender.Unspecified, matchmaking.DefaultPreferences())

		matcher := domain.NewGaleShapleyStableMatcher()
		userA1 := domain.NewUser("A1", 20, gender.Unspecified, matchmaking.DefaultPreferences())
	...
	t.Run("multiple A and B with partial compatibility", func(t *testing.T) {
		...
		userB1 := domain.NewUser("B1", 25, gender.Unspecified, prefsB1)
		userB2 := domain.NewUser("B2", 22, gender.Unspecified, prefsB2)

		matcher := domain.NewGaleShapleyStableMatcher()

		// A1 and A2 have default preferences, assuming no additional constraints
		userA1 := domain.NewUser("A1", 20, gender.Unspecified, matchmaking.DefaultPreferences())
		userA2 := domain.NewUser("A2", 24, gender.Unspecified, matchmaking.DefaultPreferences())
	...
	t.Run("no compatibility found", func(t *testing.T) {
		// B1 and B2
		userB1 := domain.NewUser("B1", 25, gender.Unspecified, matchmaking.DefaultPreferences())
		userB2 := domain.NewUser("B2", 60, gender.Male, matchmaking.DefaultPreferences())

		matcher := domain.NewGaleShapleyStableMatcher()

		// A1 and A2 have default preferences, assuming no additional constraints
		userA1 := domain.NewUser("A1", 20, gender.Male, matchmaking.DefaultPreferences().WithMaxAge(18))
		userA2 := domain.NewUser("A2", 24, gender.Female, matchmaking.DefaultPreferences().WithMaxAge(18))
